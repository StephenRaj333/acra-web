...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Define getSupportedFormat BEFORE getWebGLContext since it's called inside
    ...
    // If WebGL is not available, return early without error
    if (!webglContext) {
      console.warn('WebGL not supported, splash cursor effect disabled');
      return;
    }
    ...
    function handleMouseDown(e: MouseEvent) {
      const { x, y, isInside } = getRelativeCoords(e.clientX, e.clientY);
      if (!isInside) return;
    ...
    function handleTouchStart(e: TouchEvent) {
      ...
      for (let i = 0; i < touches.length; i++) {
        const { x, y, isInside } = getRelativeCoords(touches[i].clientX, touches[i].clientY);
  ...
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, zIndex: 0, pointerEvents: 'none', width: '100%', height: '100%', overflow: 'hidden' }}>
